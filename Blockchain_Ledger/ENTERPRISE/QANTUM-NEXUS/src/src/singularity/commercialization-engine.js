...
`);
        ...
        console.log(`   Trial Users: ${stats.trialUsers}`);
        console.log(`   Monthly Revenue: $${stats.monthlyRevenue.toLocaleString()}`);
        console.log(`   Annual Revenue: $${stats.annualRevenue.toLocaleString()}`);
        console.log('');
        console.log(`📦 PRODUCT TIERS:`);
        for (const tier of this.config.productTiers) {
            console.log(`   ${tier.name}:`);
            console.log(`      Monthly: $${tier.priceMonthly}/mo`);
            console.log(`      Yearly: $${tier.priceYearly}/yr (save ${Math.round((1 - tier.priceYearly / (tier.priceMonthly * 12)) * 100)}%)`);
    ...
    async createTrialCustomer(email, name, company) {
        console.log(`🎫 Creating trial customer: ${email}`);
    ...
    async handleSubscriptionUpdated(event) {
        ...
        if (newTier && newTier !== customer.tier) {
            ...
            customer.licenseKey = newLicense.key;
            this.licenses.set(newLicense.key, newLicense);
            console.log(`   ✅ Customer ${customer.email} upgraded to ${newTier}`);
        }
        this.saveCustomers();
    ...
    async provisionCustomer(customerId) {
        console.log(`🐳 Provisioning container for customer: ${customerId}`);
...
//# sourceMappingURL=commercialization-engine.js.map