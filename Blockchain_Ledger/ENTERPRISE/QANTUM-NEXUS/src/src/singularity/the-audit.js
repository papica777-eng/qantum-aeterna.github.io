...
`);
        ...
        console.log(`📋 Auditing ${this.config.categories.length} categories...`);
        console.log('');
    ...
    displayReport(report) {
        ...
        const certBanner = report.certification === 'CERTIFIED'
            ...
            : report.certification === 'CONDITIONALLY_CERTIFIED'
                ...
                : '❌ NOT CERTIFIED';
        console.log(`║  ${certBanner}`.padEnd(62) + '║');
        console.log(`║  ${report.certificationDetails}`.padEnd(62) + '║');
        console.log('╠═══════════════════════════════════════════════════════════════╣');
        // Statistics
        console.log('║  STATISTICS:'.padEnd(62) + '║');
        console.log(`║    Total Tests: ${report.totalTests}`.padEnd(62) + '║');
        console.log(`║    Passed: ${report.totalPassed}`.padEnd(62) + '║');
        console.log(`║    Failed: ${report.totalFailed}`.padEnd(62) + '║');
        console.log(`║    Pass Rate: ${report.passRate.toFixed(1)}%`.padEnd(62) + '║');
        console.log(`║    Overall Score: ${report.overallScore.toFixed(1)}%`.padEnd(62) + '║');
        console.log('╠═══════════════════════════════════════════════════════════════╣');
        console.log('║  CATEGORY BREAKDOWN:'.padEnd(62) + '║');
        for (const category of report.categories) {
            const percentage = (category.score / category.maxScore) * 100;
            const icon = percentage >= 95 ? '✅' : percentage >= 80 ? '⚠️' : '❌';
            const name = this.formatCategory(category.category);
            console.log(`║    ${icon} ${name}: ${percentage.toFixed(1)}%`.padEnd(62) + '║');
        }
        console.log('╠═══════════════════════════════════════════════════════════════╣');
        console.log('║  RECOMMENDATIONS:'.padEnd(62) + '║');
        for (const rec of report.recommendations) {
            console.log(`║    • ${rec.substring(0, 52)}`.padEnd(62) + '║');
        }
        console.log('╚═══════════════════════════════════════════════════════════════╝');
    }
    /**
...
//# sourceMappingURL=the-audit.js.map