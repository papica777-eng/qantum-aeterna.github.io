...
test.describe('Page: ${page.title || pageName}', () => {
    ...
    test('should load successfully', async ({ page: browserPage }) => {
        ...
        ${page.buttons.slice(0, 3).map(btn => `await expect(browserPage.getByRole('button', { name: /${this.escapeRegex(btn.text)}/i })).toBeVisible();`).join('\n        ')}
    });
...
${fillActions}
    ...
    test('should show validation errors for empty required fields', async ({ page }) => {
        ...
        ${form.fields.filter(f => f.required).map(f => `await expect(page.locator('[data-error="${f.name}"], #${f.name}-error, .error')).toBeVisible();`).join('\n        ')}
    });
...
//# sourceMappingURL=auto-test-factory.js.map