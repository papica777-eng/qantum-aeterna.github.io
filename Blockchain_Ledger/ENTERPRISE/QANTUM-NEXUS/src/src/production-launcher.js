...
`);
    ...
    if (PRODUCTION_CONFIG.memory.enableGC && typeof global.gc !== 'function') {
        console.warn('⚠️  Warning: --expose-gc flag not detected. Manual GC disabled.');
    }
    console.log('🚀 Production mode activated. System is Quantum.');
    console.log('');
    console.log('Configuration:', JSON.stringify(config, null, 2));
    // Import and start the main orchestrator
    try {
        const { SwarmOrchestrator } = await import('./swarm/swarm-orchestrator.js');
        console.log('✅ SwarmOrchestrator loaded');
        // Additional production initialization would go here
        console.log('');
        console.log('🏆 QANTUM v1.0.0.1 - READY FOR PRODUCTION');
        console.log('   "The system is Quantum."');
    }
    catch (error) {
        console.error('❌ Failed to load SwarmOrchestrator:', error);
        console.log('');
...
//# sourceMappingURL=production-launcher.js.map