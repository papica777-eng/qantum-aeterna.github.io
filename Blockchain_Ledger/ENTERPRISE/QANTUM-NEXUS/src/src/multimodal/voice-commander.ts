...
export type IntentType =
    ...
    | 'stealth_capture' // "Activate stealth mode"
    | 'sovereign_audit' // "Start sovereign audit"
...
export class VoiceCommander extends EventEmitter {
    ...
    private getDefaultVocabulary(): string[] {
        return [
            ...
            // QAntum specific
            'QANTUM', 'sovereign', 'persona', 'chaos test', 'stealth', 'ghost', 'audit', 'nexus'
        ];
    }
    ...
    private initializeIntentPatterns(): IntentPattern[] {
        return [
            ...
            // Stealth capture intents
            {
                type: 'stealth_capture',
                patterns: [
                    /^(activate|start|enable)\s+(stealth|ghost)\s+(capture|mode)?$/i,
                    /^(deactivate|stop|disable)\s+(stealth|ghost)\s+(capture|mode)?$/i,
                    /^(toggle)\s+(stealth|ghost)\s+(capture|mode)?$/i
                ],
                entityExtractors: [],
                actionBuilder: (transcript) => {
                    const active = !transcript.match(/deactivate|stop|disable/i);
                    return {
                        verb: 'screenshot',
                        target: 'stealth',
                        params: { active },
            ...
            // Sovereign audit intents
            {
                type: 'sovereign_audit',
                patterns: [
                    /^(run|start|execute|manifest)\s+(the\s+)?(sovereign\s+)?audit$/i,
                    /^(generate\s+)?(nexus\s+)?report$/i
                ],
                entityExtractors: [],
                actionBuilder: () => ({
                    verb: 'navigate', // Reusing navigate for audit trigger
                    target: 'sovereign:audit',
