...
class UnifiedServer extends events_1.EventEmitter {
    ...
    constructor(config) {
        ...
        this.logger = (0, logger_js_1.createLogger)({
            level: config.logging?.level || 'info',
            format: process.env.NODE_ENV === 'production' ? 'json' : 'pretty',
            outputs: ['console']
        });
        if (config.auth && config.auth.strategy !== 'none') {
            this.auth = new auth_js_1.AuthMiddleware(config.auth);
        }
        if (config.rateLimit) {
            this.rateLimiter = new rateLimit_js_1.TieredRateLimiter({
                windowMs: config.rateLimit.windowMs,
                tiers: config.rateLimit.tiers
            });
        }
        this.errorHandler = new errorHandler_js_1.ErrorHandler({
            environment: process.env.NODE_ENV === 'production' ? 'production' : 'development',
            includeStack: process.env.NODE_ENV !== 'production'
        });
        this.requestLogger = new logging_js_1.RequestLogger({
            logBody: config.logging?.requestBody,
            logResponseBody: config.logging?.responseBody
        });
        this.responseTimeTracker = new logging_js_1.ResponseTimeTracker();
    ...
    async start() {
        return new Promise((resolve, reject) => {
            ...
            this.server.keepAliveTimeout = this.config.timeouts.keepAlive;
        ...
        try {
            ...
            if (this.auth) {
                ...
                if (!authResult.success) {
                    throw new errorHandler_js_1.UnauthorizedError(authResult.error, authResult.errorCode);
                }
            ...
            if (this.rateLimiter && req.user) {
                ...
                if (!limitResult.allowed) {
                    throw new errorHandler_js_1.TooManyRequestsError(limitResult.retryAfter);
                }
            }
            // Find matching route
            const route = this.findRoute(method, path);
            if (!route) {
                throw new errorHandler_js_1.NotFoundError(`Route ${method} ${path}`);
            }
            // Extract path params
    ...
    async parseBody(req) {
        return new Promise((resolve, reject) => {
            ...
            req.on('data', (chunk) => {
                size += chunk.length;
                if (size > maxSize) {
                    reject(new errorHandler_js_1.BadRequestError('Request body too large'));
                    return;
                }
                chunks.push(chunk);
            });
            req.on('end', () => {
                ...
                if (contentType.includes('application/json')) {
                    try {
                        ...
                        reject(new errorHandler_js_1.BadRequestError('Invalid JSON body'));
                    }
                }
