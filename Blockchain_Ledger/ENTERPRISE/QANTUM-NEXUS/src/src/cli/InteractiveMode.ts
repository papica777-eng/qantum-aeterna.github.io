...
export class InteractiveMode extends EventEmitter {
  ...
  private async loop(): Promise<void> {
    while (this.running) {
      ...
      try {
        ...
        console.error(`${c.red}Error: ${(error as Error).message}${c.reset}`);
      }
    }
  }
  ...
  private async execute(input: string): Promise<void> {
    ...
    if (cmdName.includes('=')) {
      ...
      console.log(`${c.dim}${name} = ${this.context.variables.get(name)}${c.reset}`);
      return;
    }
...
`;
    ...
    this.register({
      ...
      execute: async (args) => {
        ...
        console.log(`${c.cyan}Running test: ${file}${c.reset}`);
        console.log(`${c.dim}... test execution would happen here ...${c.reset}`);
