...
`);
        ...
        console.log(`   ✅ Initialized with ${this.profiles.size} behavioral profiles`);
        console.log(`   ✅ Fatigue simulation: ${this.config.fatigueEnabled ? 'ENABLED' : 'DISABLED'}`);
        console.log(`   ✅ Default think-time: ${this.config.defaultThinkTimeMs}ms`);
    }
    /**
    ...
    async executeBatch(profileId, requests, options) {
        ...
        for (let i = 0; i < requests.length; i++) {
            ...
            if (session && session.currentEnergy < profile.fatigueCurve.breakThreshold) {
                console.log(`   ☕ Profile ${profileId.substring(0, 8)} taking fatigue break...`);
...
//# sourceMappingURL=behavioral-api-sync.js.map