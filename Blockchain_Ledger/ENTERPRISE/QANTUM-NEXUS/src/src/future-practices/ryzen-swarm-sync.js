...
`);
        ...
        console.log(`   🖥️ Neural Hub: ${this.localNode.cpu.model}`);
        console.log(`   💾 Memory: ${Math.round(this.localNode.memory.total / 1024 / 1024 / 1024)}GB`);
        console.log(`   🧠 Cores: ${this.localNode.cpu.cores}`);
        if (this.localNode.gpu) {
            console.log(`   🎮 GPU: ${this.localNode.gpu.model}`);
        }
        // Start heartbeat
        this.startHeartbeat();
        // Initialize sync state
        await this.synchronize();
        console.log(`   ✅ Neural Hub online and ready`);
    }
    /**
    ...
    async rebalanceTasks() {
        console.log('   ⚖️ Rebalancing tasks...');
        const tasks = Array.from(this.tasks.values());
        ...
        for (const task of queuedTasks) {
            ...
            if (task.assignedTo !== optimalTarget) {
                ...
                console.log(`   → Task ${task.taskId.substring(0, 8)} reassigned to ${optimalTarget}`);
            }
        }
    ...
    registerSwarmInstance(instance) {
        ...
        console.log(`   ☁️ Swarm instance registered: ${fullInstance.instanceId}`);
    ...
    shutdown() {
        ...
        console.log('   🛑 Ryzen-Swarm Sync shutdown');
    }
    sleep(ms) {
...
//# sourceMappingURL=ryzen-swarm-sync.js.map