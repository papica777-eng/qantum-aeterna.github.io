...
`);
        ...
        console.log('📸 Capturing initial code signatures...');
        await this.captureSignatures();
        // Start file watching
        this.isMonitoring = true;
        this.startWatching();
        console.log('👁️ Monitoring started. Watching for code changes...');
    ...
    async captureSignatures() {
        ...
        console.log(`   ✅ Captured signatures for ${this.signatures.size} files`);
    }
    /**
    ...
    async handleFileChange(filePath) {
        ...
        if (changes.length > 0) {
            ...
            console.log(`   🎯 Found ${affectedTests.length} affected test(s)`);
            // Generate evolutions
...
describe('${change.name}', () => {
    ...
    async applyEvolution(evolution) {
        try {
            ...
            console.log(`   ✅ Applied evolution to ${path.basename(evolution.testPath)}`);
            this.emit('evolution:applied', evolution);
            return true;
        }
        catch (error) {
            evolution.status = 'failed';
            console.error(`   ❌ Failed to apply evolution: ${error.message}`);
            return false;
        }
    }
    /**
...
//# sourceMappingURL=self-evolving-code.js.map