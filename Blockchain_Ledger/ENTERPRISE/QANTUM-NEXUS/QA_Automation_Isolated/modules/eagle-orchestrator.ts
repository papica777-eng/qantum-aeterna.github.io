...
class EagleOrchestrator {
  ...
  private async cmdAll(): Promise<void> {
    ...
    const mistePath = path.join(cwd, 'MisteMind.-star');
    const projects: [string, string][] = [
      ['SamsunS24', cwd],
      ...(fs.existsSync(mistePath) ? [['MisteMind', mistePath] as [string, string]] : []),
      ...(fs.existsSync('C:\\QAntumQATool') ? [['QAntumQATool', 'C:\\QAntumQATool'] as [string, string]] : []),
      ...(fs.existsSync('C:\\MisteMind') ? [['MisteMind', 'C:\\MisteMind'] as [string, string]] : []),
    ...
    if (projects.length === 0) {
      log('⚠ No valid project paths found. Using cwd only.', 'yellow');
      this.synchronizer.registerProject('SamsunS24', cwd);
    }

    this.synchronizer.compareProjects();

    log('\n', 'reset');
    for (const [name, projectPath] of projects.length ? projects : [['SamsunS24', cwd]]) {
      await this.cmdAnalyze(projectPath);
      log('\n', 'reset');
      await this.cmdGenerateDocs(projectPath, 'PROJECT-STRUCTURE.md');
    }
