...
const CONFIG = {
    // Patrol interval (ms) - колко често обикаля (30s = не претоварва)
    patrolInterval: 30000, // 30 секунди
...
class EternalPrison {
    ...
    // Освобождава при успешна поправка
    release(threatId) {
        if (this.prisoners.has(threatId)) {
            this.prisoners.delete(threatId);
            this.savePrisoners();
        }
    }
...
class EternalWatchdog {
    ...
    async neutralize(threatId, threatType, evidence) {
        ...
        const fixed = await this.takeAction(threatId, threatType, evidence);
        if (fixed) this.prison.release(threatId); // Поправихме - освобождаваме от затвора
        
        return handcuffs;
    }
    
    // 🔧 Действие според типа заплаха - РЕАЛНО ПОПРАВЯ. Връща true ако поправи.
    async takeAction(threatId, threatType, evidence) {
        ...
        switch (threatType) {
            case 'MISSING':
                const filePath = evidence.file ? path.join(root, evidence.file) : null;
                if (filePath && !fs.existsSync(filePath)) {
                    try {
                        fs.mkdirSync(path.dirname(filePath), { recursive: true });
                        const stub = this.getStubForFile(evidence.file);
                        fs.writeFileSync(filePath, JSON.stringify(stub, null, 2));
                        this.log(`✅ CREATED: ${evidence.file}`, 'INFO');
                        fixed = true;
                    } catch (e) {
                        this.log(`❌ Failed to create ${evidence.file}: ${e.message}`, 'INFO');
                    }
                }
                break;

            case 'REGISTRY_MISSING':
                const sensorPath = path.join(root, 'data', 'memoryals', 'sensor-registry.json');
                if (!fs.existsSync(sensorPath)) {
                    try {
                        fs.mkdirSync(path.dirname(sensorPath), { recursive: true });
                        fs.writeFileSync(sensorPath, JSON.stringify({ sensors: [], createdAt: Date.now() }, null, 2));
                        this.log(`✅ CREATED: data/memoryals/sensor-registry.json`, 'INFO');
                        fixed = true;
                    } catch (e) {
                        this.log(`❌ Failed to create sensor registry: ${e.message}`, 'INFO');
                    }
                }
                break;

            case 'CORRUPTED':
                const corruptPath = evidence.file ? path.join(root, evidence.file) : null;
                if (corruptPath && fs.existsSync(corruptPath)) {
                    try {
                        const stub = this.getStubForFile(evidence.file);
                        fs.writeFileSync(corruptPath, JSON.stringify(stub, null, 2));
                        this.log(`✅ RESTORED: ${evidence.file}`, 'INFO');
                        fixed = true;
                    } catch (e) {
                        this.log(`❌ Failed to restore ${evidence.file}: ${e.message}`, 'INFO');
                    }
                }
                break;
    ...
    getStubForFile(relPath) {
        if (relPath.includes('sensor-registry')) return { sensors: [], createdAt: Date.now() };
        if (relPath.includes('nerve-center-state')) return { state: 'idle', lastUpdate: Date.now() };
        if (relPath.includes('ecosystem-manifest')) return { version: 1, projects: [], createdAt: Date.now() };
        return { _stub: true, createdAt: Date.now() };
    }
    ...
    async patrol() {
        ...
        for (const issue of issues) {
            ...
            const isCritical = ['CORRUPTED', 'MISSING', 'REGISTRY_MISSING', 'HAS_ERRORS', 'PULSE_TIMEOUT'].includes(issue.issue);
...
`);
        ...
        this.patrolInterval = setInterval(async () => {
            ...
            if (result.issuesFound > 0) {
                ...
                // Silent patrol - all clear (log every 20th)
                if (result.patrolNumber % 20 === 0) {
